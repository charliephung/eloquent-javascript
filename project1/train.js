...
(function() {
  ...
  class Animation {
    constructor(worldState, robot, robotState) {
      ...
      this.map.src = "vil.png";
      this.map.style.cssText =
        "vertical-align: -8px;width: 380px; height: 270;";
    ...
    updateParcels() {
      ...
      for (let { place, address } of this.worldState.parcels) {
        ...
        node.style.cssText =
          "position: absolute; height: 16px; width: 16px; background-color: orange; background-position: 0 -" +
          offset +
          "px";
  ...
  move(destination) {
    if (!roadGraph[this.place].includes(destination)) {
      ...
      let parcels = this.parcels
        .map(p => {
          // Neu vi tri cua don hang !== vi tri cua robot
          // thi giu nguyen vi tri don hang
          if (p.place != this.place) return p;
          //   else thi vi tri don hang di theo robot
          return { place: destination, address: p.address };
        })
        // Nhung don hang nao toi dung dia diem roi thi bo
...
function view(obj) {
  console.log("Bot at: " + obj.place);
  console.log(JSON.stringify(obj.parcels));
}
...
VillageState.random = function(parcelCount = 5) {
  ...
  for (let i = 0; i < parcelCount; i++) {
    ...
    do {
      ...
      //   Dam bao ko co don hang nao trung vi tri voi dia diem den
    } while (place == address);
...
compareRobots(goalOrientedRobotOPT, [], goalOrientedRobot, []);
